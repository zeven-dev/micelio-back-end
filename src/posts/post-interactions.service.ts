...
/** Lo mínimo del post que hace falta para validar visibilidad y armar los eventos. */
interface PostRef {
  id: string;
  authorId: string;
  tags: string[];
...
/** Lo que `posts` (el resto del módulo) necesita de aquí para `viewerHasLiked/.../commentCount`. */
...
/** Marca del cursor de likes/comentarios: última fecha e id consumidos (mismo criterio que `social`). */
...
function isItemCursor(value: unknown): value is ItemCursor {
  const cursor = value as ItemCursor | null;
  return typeof cursor?.c === 'string' && typeof cursor?.id === 'string';
}

/**
 * Likes, guardados y comentarios de un post (Fase 4). Vivió en `social` hasta que el ciclo de
 * tres módulos que generaba (`users` ↔ `social` ↔ `posts`) se deshizo devolviendo esta lógica a
 * `posts`, donde ya vive el resto del contenido — decisión del dueño, ver `docs/ARCHITECTURE.md`.
 *
 * `requirePost`/`requireVisiblePost` consultan la tabla `post` directo (es el propio dominio de
 * este módulo, ya no uno ajeno) y aplican la **misma** regla de visibilidad que
 * `PostsService.findOne` vía `UsersService.canViewContentOf` — un solo camino a esa regla, no
 * dos. `save`/`unsave` no arman el `Post` completo (eso vive en `PostsService.listSaved`, que sí
 * lo necesita); aquí solo cambia el estado de la fila `saved_posts`.
 */
@Injectable()
export class PostInteractionsService {
  ...
  /**
   * `viewerHasLiked/viewerHasSaved/likeCount/commentCount` de varios posts a la vez — el resto
   * de `posts` la llama para armar `PostResponseDto`, en una sola pasada agregada por página
   * (mismo criterio que `UsersService.getPublicViewsByIds`), nunca consultando `likes`/
   * `saved_posts`/`comments` post por post.
   */
  ...
  async listLikes(
    ...
    const cursor = query.cursor ? decodeCursor(query.cursor, isItemCursor) : null;
